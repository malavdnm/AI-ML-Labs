...
def grade2(trainX, trainY, testX, testY):
	print('='*20 + ' TASK 2 - Ordinary Least Squares' + '='*20)
	try:
		_lambda = 0
		W_stud = ordinary_least_squares(trainX, trainY)
...
def grade3(trainX, trainY, testX, testY):
	print('='*20 + ' TASK 3 - PartA-  Ridge Regression' + '='*20)
	try:
		_lambda = 100
		W_stud = ridge_grad_descent(trainX, trainY, _lambda)
		W_act = np.linalg.inv(trainX.T @ trainX + _lambda * np.eye(trainX.shape[1])) @ trainX.T @ trainY
		W_stud1 = ordinary_least_squares(trainX, trainY)
		if np.linalg.norm(W_stud - W_act) < 1 and sse(trainX, trainY, W_stud)>sse(trainX, trainY, W_stud1):
			marks = 1
			print("TASK 3 Passed")
		else:
			marks = 0
...
if __name__ == "__main__":
	...
	if sys.argv[1].lower() == 'all':
		...
		m3 = grade3(trainX, trainY, testX, testY)
		...
		m3 = grade3(trainX, trainY, testX, testYf)
