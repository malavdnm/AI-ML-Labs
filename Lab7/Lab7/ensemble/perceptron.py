...
class PerceptronClassifier:
    ...
    def train( self, trainingData, trainingLabels, sample_weights=None):
        ...
        for iteration in range(self.max_iterations):
            for i in range(len(trainingData)): 
                "*** YOUR CODE HERE ***"
                pred_label=self.classify([trainingData[i]])[0]
    ...
    def sample_data(self, trainingData, trainingLabels, sample_weights):
        "*** YOUR CODE HERE ***"
        n = len(trainingData)
        index_arr = np.random.choice(n, size=int(n * 0.85), replace=True, p=sample_weights)
        # index_arr = util.nSample(sample_weights, list(range(n)), int(n*0.55))
