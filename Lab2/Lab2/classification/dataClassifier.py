...
def enhancedFeatureExtractorDigit(datum):
    ...
    1. Number of nonzero pixels
    2. Percentage of active pixels above the horizontal center line
    3. Percentage of active pixels to the right of the vertical center line
    4. Number of discontinuity in the pixel values in vertical and horizontal direction
    5. Estimated aspect ratio of the image i.e. width/height
    ##
    """
    features = util.Counter()
    breaks = 0
    ...
    firstLeft = 0
    aboveCenter = 0
    #arr[:,1:DATUM_WIDTH] and transversing in horizontal direction
    for i in range(DATUM_HEIGHT):
        for j in range(1,DATUM_WIDTH):
            if datum[i * DATUM_HEIGHT + j] != 0:
                nonzero += 1
                if not firstLeft or j < firstLeft:
                    firstLeft = j
                if j <= (DATUM_HEIGHT + 1) / 2:
                    aboveCenter += 1
            if datum[i * DATUM_HEIGHT + j] != datum[i * DATUM_HEIGHT + j-1]:
                breaks += 1
    #arr[0,0]            
    if datum[0]!=0:
        nonzero += 1

    width = DATUM_WIDTH - (firstLeft * 2)

    #arr[1:DATUM_HEIGHT,:] and transversing in vertical direction
    firstTop = 0
    pastRight = 0
    for j in range(DATUM_WIDTH):
        for i in range(1,DATUM_HEIGHT):
            if datum[i * DATUM_HEIGHT] != 0:
                nonzero += 1
            if datum[i * DATUM_HEIGHT + j] != 0:
                if not firstTop or i < firstTop:
                    firstTop = i
                if j <= (DATUM_WIDTH + 1) / 2:
                    pastRight += 1
            if datum[i * DATUM_HEIGHT] != datum[i * DATUM_HEIGHT-1]:
                breaks += 1

    height = DATUM_HEIGHT - (2*firstTop)
    aspectRatio = float(width) / height
    percentAbove = float(aboveCenter) / nonzero
    percentRight = float(pastRight) / nonzero
    extract_features=[breaks,aspectRatio,nonzero,percentAbove,percentRight]
    for i in range(len(extract_features)):
        features[i]=extract_features[i]
