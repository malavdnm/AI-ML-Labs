...
def kmeans_iteration_one(data, centroids, distance):
	...
	for point in data:
		distances=[distance(point,k) for k in centroids]
		nearest_centroid=argmin(distances)
		new_clustering[nearest_centroid].append(point)

	for i in range(len(new_clustering)):
		if len(new_clustering[i])==0:
			new_clustering[i].append(centroids[i])
	
	new_centroids = [tuple(map(avg, zip(*points))) for points in new_clustering]
	########################################
...
def hasconverged(old_centroids, new_centroids, epsilon=1e-1):
	...
	converged = sum([distance_euclidean(c1, c2) > epsilon for c1, c2 in zip(old_centroids, new_centroids)]) == 0
	########################################
	return converged
...
def performance_SSE(data, centroids, distance):
	...
	sse = sum(map(lambda point: min(map(lambda c: distance(point, c), centroids)) ** 2, data))
	########################################
	return sse
